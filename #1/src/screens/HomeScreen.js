...
import { collection, onSnapshot, orderBy, query, doc, updateDoc, arrayUnion, arrayRemove, increment } from 'firebase/firestore';
    ...
    if (nextIndex < list.length) {
      ...
      if (isVideo && nextUriCandidate && !prefetchingRef.current[nextUriCandidate]) {
        ...
        FileSystem.getInfoAsync(fileUri).then(info => {
          if (!info.exists) {
            FileSystem.downloadAsync(nextUriCandidate, fileUri).catch(()=>{});
          }
        }).catch(()=>{});
      }
    }
  ...
  useEffect(() => {
    ...
    targets.forEach(i => {
      ...
      if (isVideo && uriCandidate) {
        fetch(uriCandidate, { method: 'HEAD' }).catch(()=>{});
      }
    });
  }, [currentIndex, currentDiscoverIndex, selectedTab, videos, randomPosts]);
  ...
  const renderHeader = () => (
    ...
    >
      <View style={styles.headerTop}>
        <TouchableOpacity style={styles.menuButton} onPress={handleAddTestPosts}>
...
const styles = StyleSheet.create({
  ...
  tabText: {
    color: '#9ca3af',
    fontSize: responsiveFont(11), // Smaller for Dad's device
    fontWeight: '600',
    includeFontPadding: false, // Reduce padding on Android
  },
  activeTabText: {
    color: '#ffffff',
  },
