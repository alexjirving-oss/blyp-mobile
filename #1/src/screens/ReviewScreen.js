...
const ReviewScreen = () => {
  ...
  useEffect(() => {
    ...
    if (mediaItems.length > 0 && (type === 'photo' || type === 'photos') && !showDescriptionMethodOverlay) {
      // If coming from camera with multiple photos, skip description overlay and go straight to step-by-step
      if (type === 'photos' && mediaItems.length > 1) {
        console.log('🚀 Multiple photos from camera - going straight to step-by-step processing');
        // No auto-processing - always show overlay
        console.log('📸 Showing description overlay instead of auto-processing');
      } else {
        console.log('� Immediately showing description overlay for single photo from camera');
        // Show description overlay for photos
