...
/** Pause after result before next auto-spin (owner walk-away: 60s default). */
const AUTO_DELAY_OPTS = [5, 10, 15, 30, 45, 60];
/** Prize chips — default 10, walk-away option 20. */
const PRIZE_OPTS = [10, 20, 25, 50];
...
function ChipRow({ label, opts, valueSec, onPick, disabled, hint, suffix = 's' }) {
...
export default function FrenemiesSettingsSheet({
  ...
  const [throwText, setThrowText] = useState(String(settings?.throwCoins ?? 10));
  const [soloText, setSoloText] = useState(String(settings?.soloCoins ?? 10));
  ...
  useEffect(() => {
    ...
    if (justOpened && settings) {
      setDraft(settings);
      setThrowText(String(settings.throwCoins ?? 10));
      setSoloText(String(settings.soloCoins ?? 10));
  ...
  const autoDelaySec = nearestOpt(
    Math.round((draft.autoContinueDelayMs || 60000) / 1000),
    AUTO_DELAY_OPTS
  );
  ...
  const commitCoins = (field, text) => {
    ...
    const apply = () => {
      ...
      if (field === 'throwCoins') setThrowText(String(n));
      if (field === 'soloCoins') setSoloText(String(n));
    };
    if (n > 100) {
      Alert.alert(
        'High prize amount',
            ...
            >
              ...
              {readOnly ? (
                <Text style={styles.readOnly} allowFontScaling={false}>
                  View only mid-round — Auto-continue can still change.
                </Text>
              ) : null}
              ...
              >
                ...
                <ChipRow
                  label="Spin duration"
                  hint="Smooth ease-out coast — longer = more ticks"
                  opts={SPIN_OPTS}
                  valueSec={spinSec}
                  disabled={readOnly}
                ...
                <RowToggle
                  ...
                  hint={
                    isAdminHost
                      ? 'ON = walk away. Auto-spin, auto seat fill/drop each round. House pays if enabled.'
                      : 'ON = walk away. Auto-spin + auto seat fill/drop. Prizes from your balance.'
                  }
                  ...
                  onValueChange={(v) => {
                    ...
                    setDraft((d) => ({
                      ...d,
                      autoContinue: next,
                      // Prefer walk-away cadence when first enabling.
                      autoContinueDelayMs:
                        next && !(d.autoContinueDelayMs >= 30_000)
                          ? 60_000
                          : d.autoContinueDelayMs || 60_000,
                    }));
                  }}
                />
                ...
                {draft.autoContinue === true ? (
                  <ChipRow
                    label="Auto-spin interval"
                    hint="Pause after each result before the next spin (~60s walk-away)"
                ...
                <ChipRow
                  label="Coins per successful throw"
                  hint="Default 10 · pick 20 for bigger auto rounds"
                  opts={PRIZE_OPTS}
                  valueSec={draft.throwCoins ?? 10}
                  disabled={readOnly}
                  suffix=""
                  onPick={(n) => {
                    setThrowText(String(n));
                    setDraft((d) => ({ ...d, throwCoins: n }));
                  }}
                />
                <Text style={styles.fieldLabel} allowFontScaling={false}>
                  Custom throw amount
                ...
                <ChipRow
                  label="Coins per solo challenge win"
                  opts={PRIZE_OPTS}
                  valueSec={draft.soloCoins ?? 10}
                  disabled={readOnly}
                  suffix=""
                  onPick={(n) => {
                    setSoloText(String(n));
                    setDraft((d) => ({ ...d, soloCoins: n }));
                  }}
                />
                <Text style={styles.fieldLabel} allowFontScaling={false}>
                  Custom solo amount
                ...
                />
                <Text style={styles.hint} allowFontScaling={false}>
                  Challenge → throw uses throw amount once. Max 500. Confirm above 100. Real spendable COIN ledger grants (House or host paid hold).
                </Text>

                {isAdminHost ? (
                  <RowToggle
                    label="House pays"
                    hint="Admin / owner — prizes from House wallet (recommended for walk-away auto)"
