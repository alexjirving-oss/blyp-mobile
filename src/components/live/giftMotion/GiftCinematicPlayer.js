/**
 * GiftCinematicPlayer — Disney/Pixar-bar theatrical gift cinema shell.
 *
 * Preload → entrance → impact → climax → glory hold → elegant dismiss.
 * Skia hero scenes + letterbox + stage grade + diegetic plaque + tap-skip + haptics.
 *
 * Honest: this is in-app theatrical cinema, not film EXR/USD renders.
 */
...
export default function GiftCinematicPlayer({ entry, onSkip, onDone }) {
  ...
  const skippable = true; // all V2 cinemas are skippable
