...
export function logStreamingEvent(
  ...
  if (event.includes('FAILURE') || event.includes('ERROR')) {
    // NOTE: Many failures are expected in dev (logged out, backend down, token resets).
    // Using console.error triggers a red LogBox which looks like an app crash.
    // Keep visibility via warn/log instead.
    const reasonText = String(payload.reason || payload.errorMessage || '');
    const isAuthRelated =
      reasonText.includes('COGNITO_JWT') ||
      reasonText.includes('NOT_LOGGED_IN') ||
      reasonText.toLowerCase().includes('token');
    if (isAuthRelated) {
      console.log(consolePrefix, consolePayload);
    } else {
      console.warn(consolePrefix, consolePayload);
    }
