 ...
 */
function getRegionFromStageArn(stageArn) {
    ...
    if (parts.length < 6 || parts[0] !== 'arn' || parts[2] !== 'ivs') {
        throw new Error(`Invalid IVS stage ARN format: ${stageArn}. ` +
...
class IvsRealtimeService {
    ...
    getClientForRegion(region) {
        if (!this.clientsByRegion.has(region)) {
            ...
            this.clientsByRegion.set(region, new client_ivs_realtime_1.IVSRealTimeClient({ region }));
        }
    ...
    async createParticipantToken(opts) {
        ...
        try {
            ...
            throw new Error(`Invalid stage ARN: ${error instanceof Error ? error.message : String(error)}`);
        }
        ...
        try {
            ...
            const command = new client_ivs_realtime_1.CreateParticipantTokenCommand(input);
            const response = await client.send(command);
            if (!((_a = response.participantToken) === null || _a === void 0 ? void 0 : _a.token)) {
            ...
            throw new Error(`Failed to create IVS participant token: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
exports.IvsRealtimeService = IvsRealtimeService;
/**
...
exports.IvsViewerService = IvsViewerService;
// Singleton instances
...
exports.ivsRealtime = new IvsRealtimeService(process.env.AWS_REGION || 'eu-west-1');
// Lazy-initialize viewer service to allow emulator to start without IVS_PLAYBACK_URL
exports.ivsViewer = null;
...
function initializeViewerService() {
    if (exports.ivsViewer) {
        return exports.ivsViewer;
    }
    try {
        const playbackUrl = getPlaybackUrl();
        exports.ivsViewer = new IvsViewerService(playbackUrl);
