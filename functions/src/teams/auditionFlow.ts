...
async function teamMeta(db: Fs, teamId: string): Promise<{ name: string; leaderId: string | null }> {
  try {
    const snap = await db.collection('teams').doc(teamId).get();
    const d = (snap.exists ? snap.data() : null) as any;
    return { name: d?.name || 'the agency', leaderId: d?.leaderId || null };
  } catch {
    return { name: 'the agency', leaderId: null };
  }
}

async function userProfile(db: Fs, uid: string): Promise<{ displayName: string; photoURL: string | null }> {
  try {
    const snap = await db.collection('users').doc(uid).get();
    const d = (snap.exists ? snap.data() : null) as any;
    const displayName = d?.displayName || d?.username || d?.name || 'Member';
    const photoURL = d?.photoURL || d?.avatar || d?.userPhotoURL || d?.photo || null;
...
async function fallbackAccountUid(db: Fs): Promise<string | null> {
  ...
  try {
    ...
    const d = (snap.exists ? snap.data() : null) as any;
    const uid = String(d?.auditionFallbackUid || '').trim();
    return uid || null;
  } catch {
    return null;
  }
 ...
 */
export const onAuditionMatched = functions.firestore
  .document('auditions/{auditionId}')
  .onUpdate(async (change, context) => {
    initFirebaseAdmin();
    const db = admin.firestore();
    const { auditionId } = context.params as { auditionId: string };
    const before = change.before.data() as any;
    const after = change.after.data() as any;
    if (!after) return null;

    const becameMatched = before?.status !== 'matched' && after.status === 'matched';
    if (!becameMatched) return null;
    if (after.battleId) return null; // already has a battle
    if (!after.aUid || !after.bUid) return null;
 ...
 * battle.
 */
export const auditionOpponentSweep = functions.pubsub
  ...
  .onRun(async () => {
    ...
    for (const doc of snap.docs) {
      ...
      let oppUid: string | null = null;
      let fallbackRole: 'leader' | 'alex' | null = null;
 ...
 */
export const onAuditionBattleComplete = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (!after.isAudition || !auditionId) return null;
    ...
    if (!audSnap.exists) return null;
    const aud = audSnap.data() as any;
 ...
 */
export const onAuditionDecision = functions.firestore
  .document('auditions/{auditionId}')
  .onUpdate(async (change, context) => {
    initFirebaseAdmin();
    const db = admin.firestore();
    ...
    const changed: Array<{ uid: string; decision: string }> = [];
