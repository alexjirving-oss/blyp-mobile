...
async function purgePosts(uid: string): Promise<number> {
  let deleted = 0;
  ...
  for (const docSnap of snap.docs) {
    ...
    for (const f of STORAGE_FIELDS) {
      ...
      if (p && typeof p === 'string' && !p.startsWith('http')) {
        try { await bucket.file(p).delete({ ignoreNotFound: true }); } catch { /* ignore */ }
      }
    }
    try { await docSnap.ref.delete(); deleted++; } catch { /* ignore */ }
  }
  return deleted;
}

async function purgeStoragePrefixes(uid: string): Promise<void> {
  const bucket = getBucket();
  const prefixes = [`posts/${uid}/`, `avatars/${uid}/`, `profiles/${uid}/`, `uploads/${uid}/`, `media/${uid}/`];
  for (const prefix of prefixes) {
    try { await bucket.deleteFiles({ prefix, force: true }); } catch { /* ignore */ }
  }
}

async function purgeActivities(uid: string): Promise<number> {
  let deleted = 0;
  for (const field of ['actorId', 'targetUserId']) {
    try {
      const snap = await db.collection('activities').where(field, '==', uid).limit(500).get();
      for (const d of snap.docs) {
        try { await d.ref.delete(); deleted++; } catch { /* ignore */ }
      }
    } catch { /* ignore (missing index etc.) */ }
  }
  return deleted;
}

async function purgeBackend(uid: string): Promise<{ ok: boolean; deleted?: any; cognitoDeleted?: boolean }> {
  ...
  try {
    ...
    return { ok: true, deleted: data?.deleted, cognitoDeleted: Boolean(data?.cognitoDeleted) };
  } catch (e) {
    console.error('[deletionWorker] backend purge error', (e as Error)?.message);
    return { ok: false };
  }
}
...
export const processAccountDeletion = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      try { await db.collection('userProfiles').doc(uid).delete(); } catch { /* ignore */ }
      ...
      console.error('[deletionWorker] purge failed for', uid, e?.message || e);
      try {
        await reqRef.set({ status: 'failed', failedAt: Date.now(), error: String(e?.message || e).slice(0, 500) }, { merge: true });
      } catch { /* ignore */ }
    }
  });
