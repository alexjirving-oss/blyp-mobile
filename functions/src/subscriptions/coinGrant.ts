 ...
 */
export async function creditSubscriptionCoinsViaLiveService(params: {
  uid: string;
  coins: number;
  ...
  if (!base || !secret) {
    console.error(
      '[subscriptions] coin grant SKIPPED: LIVE_SERVICE_BASE_URL / INTERNAL_SHARED_SECRET not configured',
    );
  ...
  try {
    ...
    return { ok: true, granted: Number(data?.granted || 0) };
  } catch (e) {
    console.error('[subscriptions] coin grant error', (e as Error)?.message);
    return { ok: false, granted: 0 };
  }
}
